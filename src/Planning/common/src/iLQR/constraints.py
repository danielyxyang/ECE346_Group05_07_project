...
class Constraints:
	...
	def _road_boundary_cost(self, states: np.ndarray, closest_pt: np.ndarray,	slope: np.ndarray) -> np.ndarray:
	...
	def _road_boundary_derivatie(self, states: np.ndarray, closest_pt: np.ndarray, slope: np.ndarray) -> np.ndarray:
		"""
	...
	def barrier_function(self, q1: float, q2: float, c: np.ndarray, c_dot: np.ndarray) -> np.ndarray:
		"""
    c = [n] array
    c_dot = [dxn] array
    """
